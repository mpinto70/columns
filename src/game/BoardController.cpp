#include "BoardController.h"

#include "state/ScoreBoard.h"
#include "util/Random.h"

#include <map>
#include <stdexcept>
#include <utility>

namespace game {

BoardController::BoardController(piece::SharedBoard board, const size_t max_sub_row)
      : board_(board),
        max_sub_row_(max_sub_row),
        possibles_{ gui::Color::GREEN, gui::Color::RED, gui::Color::BLUE, gui::Color::YELLOW, gui::Color::LAVENDER } {
    if (max_sub_row_ == 0) {
        throw std::invalid_argument("BoardController - zero maximum subdivisions");
    }
}

bool BoardController::add_piece(const piece::Piece& piece) {
    if (has_piece()) {
        throw std::logic_error("BoardController::add_piece - there is a piece falling");
    }

    const size_t c = determine_column();
    if (c == board_->width()) {
        return false;
    }

    for (unsigned char i = 0; i < piece::PIECE_SIZE; ++i)
        if (piece[i] == gui::Color::NONE) {
            throw std::invalid_argument("BoardController::add_piece - piece with background color");
        }

    piece_position_.reset(new piece::PiecePosition(*board_, c, max_sub_row_));
    piece_.reset(new piece::Piece(piece));
    return true;
}

const piece::PiecePosition& BoardController::piece_position() const {
    if (not has_piece()) {
        throw std::logic_error("BoardController::piece_position - there is no piece falling");
    }
    return *piece_position_;
}

const piece::Piece& BoardController::piece() const {
    if (not has_piece()) {
        throw std::logic_error("BoardController::piece - there is no piece falling");
    }
    return *piece_;
}

void BoardController::step() {
    if (not has_piece()) {
        return;
    }

    const size_t row_piece = piece_position_->row();
    const size_t column_piece = piece_position_->column();
    if (reached_end()) {
        const piece::Piece& piece = *piece_;
        for (unsigned char i = 0; i < piece::PIECE_SIZE; ++i) {
            board_->at(column_piece, row_piece + i) = piece[i];
        }
        piece_.reset();
        piece_position_.reset();
    } else { // there is still space to fall
        piece_position_->step_down();
    }
}

void BoardController::move_left() {
    if (not has_piece()) {
        return;
    }

    const size_t piece_column = piece_position_->column();
    if (can_move_to(piece_column - 1)) {
        piece_position_->move_left();
    }
}

void BoardController::move_right() {
    if (not has_piece()) {
        return;
    }

    const size_t piece_column = piece_position_->column();
    if (can_move_to(piece_column + 1)) {
        piece_position_->move_right();
    }
}

void BoardController::roll_up() {
    if (not has_piece()) {
        return;
    }

    piece_->roll_up();
}

void BoardController::roll_down() {
    if (not has_piece()) {
        return;
    }

    piece_->roll_down();
}

piece::Board::EliminationList BoardController::determine_elimination() const {
    return board_->elimination_list();
}

void BoardController::eliminate(const piece::Board::EliminationList& casas) {
    for (size_t i = 0; i < casas.size(); ++i) {
        const size_t col = casas[i].column;
        const size_t row = casas[i].row;
        board_->remove(col, row);
    }
}

bool BoardController::can_move_to(size_t column) const {
    if (not has_piece()) {
        return false;
    }

    if (column >= board_->width()) {
        return false;
    }

    const size_t piece_row = piece_position_->row();
    const size_t piece_sub_row = piece_position_->sub_row();

    for (size_t r = piece_row; r < piece_row + piece::PIECE_SIZE; ++r) {
        if (board_->used(column, r)) {
            return false;
        }
    }
    if (piece_sub_row > 0
          && board_->used(column, piece_row + piece::PIECE_SIZE)) {
        return false;
    }

    return true;
}

size_t BoardController::determine_column() const {
    std::vector<size_t> available;
    for (size_t i = 0; i < board_->width(); ++i) {
        if (can_add_piece(i)) {
            available.push_back(i);
        }
    }
    if (available.size() == 0) {
        return board_->width();
    } else {
        const size_t idx = util::Random::get(available.size() - 1);
        return available[idx];
    }
}

bool BoardController::can_add_piece(size_t column) const {
    for (unsigned char i = 0; i < piece::PIECE_SIZE; ++i)
        if (board_->used(column, i)) {
            return false;
        }
    return true;
}

bool BoardController::reached_end() const {
    using namespace std::rel_ops;
    if (not has_piece()) {
        return false;
    }

    if (piece_position_->reached_bottom()) {
        return true;
    }

    const size_t piece_sub_row = piece_position_->sub_row();

    // it is still in middle of a tile
    if (piece_sub_row != 0) {
        return false;
    }

    const size_t piece_row = piece_position_->row();
    const size_t piece_column = piece_position_->column();
    // is over a non background tile
    if (board_->used(piece_column, piece_row + piece::PIECE_SIZE)) {
        return true;
    }

    return false;
}
}
