...
state::EliminationList BoardController::determine_elimination() const {
    ...
    for (size_t c = 0; c < board_->width(); ++c) {
        for (size_t l = 0; l < board_->height(); ++l) {
...
void BoardController::eliminate(const state::EliminationList& casas) {
    for (size_t i = 0; i < casas.size(); ++i) {
        const size_t col = casas[i].first;
        const size_t row = casas[i].second;
...
bool BoardController::should_eliminate(const size_t column, const size_t row) const {
    const gui::Color color = board_->at(column, row);
    if (color == board_->background_color()) {
        return false;
    }
