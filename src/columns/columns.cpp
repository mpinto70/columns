...
const std::array<gui::Color, 5> POSSIBLES = {
    gui::Color::GREEN, gui::Color::RED, gui::Color::BLUE, gui::Color::YELLOW, gui::Color::LAVENDER,
};
...
int run() {
    try {
        ...
        auto canvas = std::make_unique<graphics::sdl::Canvas>(
              *board,
        ...
        auto piece_creator = std::make_unique<piece::CreatorRandom>(
              board,
              std::vector<gui::Color>(POSSIBLES.begin(), POSSIBLES.end()));
